...
class CategoryDropdown extends StatelessWidget {
        ...
        child: DropdownButton2(
          ...
          customButton: Align(
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    Text(category, style: const TextStyle(fontSize: 16, color: Color(0xfffd894f))),
                    const Spacer(),
                    Icon(
  ...
  List<DropdownMenuItem<String>> _buildMenuItems() {
    final divisions = ["Showing all topics"];
    divisions.addAll(Milestone.milestoneCategories.map((category) =>category.name ));
    ...
    for (int i = 0; i < divisions.length; i++) {
      menuItems.add(DropdownMenuItem(
        ...
        child: Container(
          ...
          color: divisions[i] == category ? const Color(0xfffd894f).withOpacity(0.2) : Colors.white,
  ...
  List<double> _getCustomItemsHeights() {
    ...
    for (var i = 0; i < ((Milestone.milestoneCategories.length + 1) * 2) - 1; i++) {
      if (i.isEven) {
        itemsHeights.add(40);
      }
